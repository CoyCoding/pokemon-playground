...
for (var i = 0, len = draggableElems.length; i < len; i++) {
	var draggieElem = draggableElems[i];
	...
	draggie.on('dragEnd', function(event) {
		...
		this.position.x = applyGrid(this.position.x, grid[0]);

		this.position.y = applyGrid(this.position.y, grid[1]);
		this.setLeftTop();
	});

	function applyGrid(value, grid) {
