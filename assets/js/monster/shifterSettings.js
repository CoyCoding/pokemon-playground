export function shapeshiftInit() {
	$('.grid').shapeshift({
		align: 'left',
		gutterX: 0,
		gutterY: 10,
	...
	function shapeshiftClearFix() {
		$('.clear-fix').removeClass('ss-active-child');
	}
}
	...
	(function($, window, document) {
		...
		Plugin = (function() {
				...
				if (options.colWidth === null) {
					...
					if ($children.length === 0) {
						...
						console.error(
							'' +
								error_msg +
								' option colWidth must be specified if Shapeshift is initialized with no active children.'
						);
					}
				}
				return !errors;
			...
			Plugin.prototype.createEvents = function() {
				...
				$container
					.off('ss-arrange')
					.on('ss-arrange', function(e, trigger_drop_finished) {
				...
				$container
					.off('ss-setTargetPosition')
					.on('ss-setTargetPosition', function() {
			...
			Plugin.prototype.setActiveChildren = function() {
				...
				_results = [];
				for (
					i = _j = 0, _ref = this.parsedChildren.length;
			...
			Plugin.prototype.setGridColumns = function() {
				...
				columns =
					options.columns || Math.floor((inner_width + gutter_x) / col_width);
				...
				if (columns > children_count) {
					actual_columns = 0;
					for (
						i = _i = 0, _ref = this.parsedChildren.length;
			...
			Plugin.prototype.arrange = function(reparse, trigger_drop_finished) {
				...
				animated =
					globals.animated && total_children <= options.animationThreshold;
				animation_speed = options.animationSpeed;
				dragged_class = options.draggedClass;
				for (
					i = _i = 0;
					0 <= total_children ? _i < total_children : _i > total_children;
					i = 0 <= total_children ? ++_i : --_i
				) {
					...
					if (animated && !is_dragged_child) {
						$child
							.stop(true, false)
							.animate(attributes, animation_speed, function() {});
					} else {
			...
			Plugin.prototype.getPositions = function(include_dragged) {
				...
				savePosition = function(child) {
					...
					if (colspan >= 1) {
						...
						for (
							j = _j = 1;
							1 <= colspan ? _j < colspan : _j > colspan;
							j = 1 <= colspan + 100 ? ++_j : --_j
						) {
				...
				determineMultiposition = function(child) {
					...
					for (
						offset = _j = 0;
						0 <= possible_cols ? _j < possible_cols : _j > possible_cols;
						offset = 0 <= possible_cols ? ++_j : --_j
					) {
						...
						for (
							span = _k = 1;
							1 <= colspan ? _k < colspan : _k > colspan;
							span = 1 <= colspan ? ++_k : --_k
						) {
				...
				recalculateSavedChildren = function() {
					...
					to_pop = [];
					for (
						saved_i = _j = 0, _ref1 = saved_children.length;
						0 <= _ref1 ? _j < _ref1 : _j > _ref1;
						saved_i = 0 <= _ref1 ? ++_j : --_j
					) {
					...
					_results = [];
					for (
						pop_i = _k = _ref2 = to_pop.length - 1;
						_k >= 0;
						pop_i = _k += -1
					) {
						index = to_pop[pop_i];
				...
				(determinePositions = function() {
					...
					for (
						i = _j = 0;
						0 <= total_children ? _j < total_children : _j > total_children;
						i = 0 <= total_children ? ++_j : --_j
					) {
			...
			Plugin.prototype.enableDragNDrop = function() {
				...
				function getMouseStartX() {
					return startx;
				}

				function getMouseStartY() {
					return starty;
				}
				...
				if (options.enableDrag) {
					$container
						.children('.' + active_class)
						.filter(options.dragWhitelist)
						.draggable({
							addClasses: false,
							...
							start: function(e, ui) {
								...
								$selected
									.parent()
									.addClass(original_container_class)
									.addClass(current_container_class);
								console.log($selected.context.offsetLeft);
								selected_offset_y = $selected.context.offsetTop;
								return (selected_offset_x = $selected.context.offsetLeft);
							},
							drag: function(e, ui) {
								if (
									!drag_timeout &&
								...
								ui.position.left =
									getMouseX() - getMouseStartX() + selected_offset_x;
								return (ui.position.top =
									getMouseY() - getMouseStartY() + selected_offset_y);
							},
							stop: function() {
								...
								if (drag_clone) {
									if (
										...
										$original_container.shapeshift(
											$original_container.data('plugin_shapeshift').options
										);
										$current_container.shapeshift(
											$current_container.data('plugin_shapeshift').options
										);
									}
								}
								...
								$original_container
									.trigger('ss-arrange')
									.removeClass(original_container_class);
								$current_container
									.trigger('ss-arrange', true)
									.removeClass(current_container_class);
								$previous_container
									.trigger('ss-arrange')
									.removeClass(previous_container_class);
				...
				if (options.enableCrossDrop) {
					return $container.droppable({
						...
						tolerance: 'intersect',
						over: function(e) {
							$('.' + previous_container_class).removeClass(
								previous_container_class
							);
							$('.' + current_container_class)
								.removeClass(current_container_class)
								.addClass(previous_container_class);
			...
			Plugin.prototype.setTargetPosition = function() {
				...
				if (!options.enableTrash) {
					...
					selected_x =
						...
						$start_container.offset().left +
						this.globals.col_width / 2;
					selected_y =
						$selected.offset().top -
						$start_container.offset().top +
						$selected.height() / 2;
					...
					if (total_positions > 1) {
						...
						for (
							position_i = _i = cutoff_start;
							cutoff_start <= cutoff_end ? _i < cutoff_end : _i > cutoff_end;
							position_i = cutoff_start <= cutoff_end ? ++_i : --_i
						) {
					...
					if ($start_container[0] !== $selected.parent()[0]) {
						previous_container_class = options.previousContainerClass;
						if (
							$('.' + previous_container_class).data('plugin_shapeshift')
								.options.enableCrossDrop == true
						) {
		...
		return ($.fn[pluginName] = function(options) {
			return this.each(function() {
				...
				old_class =
					(_ref = $(this).attr('class')) != null
						? (_ref1 = _ref.match(/shapeshifted_container_\w+/)) != null
							? _ref1[0]
							: void 0
						: void 0;
				if (old_class) {
